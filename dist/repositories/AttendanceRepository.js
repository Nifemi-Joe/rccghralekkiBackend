...
class AttendanceRepository {
    ...
    // CHECK-IN / CHECK-OUT
    // ============================================================================
    async checkin(churchId, data, checkedInBy, isFirstTime = false) {
    ...
    async checkout(churchId, attendanceId) {
        const query = `
            ...
            RETURNING *,
                (SELECT CONCAT(first_name, ' ', last_name) FROM members WHERE id = member_id) as member_name
        `;
        const result = await database_1.pool.query(query, [attendanceId, churchId]);
        return result.rows[0] ? this.mapToAttendance(result.rows[0]) : null;
    }
    async bulkCheckin(churchId, eventInstanceId, memberIds, checkedInBy) {
        const attendances = [];
        for (const memberId of memberIds) {
            const isFirstTime = await this.isFirstTimeAttendee(churchId, memberId);
            const attendance = await this.checkin(churchId, {
    ...
    // QUERIES
    // ============================================================================
    async findExistingCheckin(churchId, eventInstanceId, memberId) {
        const query = `
            ...
            WHERE church_id = $1 
              ...
              AND member_id = $3
        `;
        const result = await database_1.pool.query(query, [churchId, eventInstanceId, memberId]);
        return result.rows[0] ? this.mapToAttendance(result.rows[0]) : null;
    }
    async findByEventInstance(churchId, eventInstanceId, filters) {
        let query = `
            SELECT a.*,
        ...
        `;
        const params = [churchId, eventInstanceId];
    ...
    async isFirstTimeAttendee(churchId, memberId) {
        const query = `
            SELECT COUNT(*) as count 
            FROM attendance 
            WHERE church_id = $1 AND member_id = $2
        `;
        const result = await database_1.pool.query(query, [churchId, memberId]);
        return parseInt(result.rows[0].count) === 0;
    }
    // ============================================================================
    // STATISTICS
    // ============================================================================
    async getEventInstanceStats(churchId, eventInstanceId) {
        const query = `
            SELECT 
        ...
        `;
        const result = await database_1.pool.query(query, [churchId, eventInstanceId]);
        const stats = {
            total_attendance: 0,
            unique_members: 0,
            guests: 0,
    ...
    async getStatistics(churchId, options) {
        ...
        `;
        const avgResult = await database_1.pool.query(avgQuery, [churchId]);
        ...
        const trendQuery = `
            ...
            WHERE church_id = $1
        `;
        const trendResult = await database_1.pool.query(trendQuery, [churchId]);
    ...
    async getAttendanceTrends(churchId, period, months) {
        ...
        `;
        const result = await database_1.pool.query(query, [churchId, months, interval]);
    ...
    async getMemberAttendanceHistory(churchId, memberId, options) {
        ...
        const memberQuery = `
            ...
            WHERE id = $1 AND church_id = $2
        `;
        const memberResult = await database_1.pool.query(memberQuery, [memberId, churchId]);
        ...
        `;
        const params = [churchId, memberId];
        if (options?.startDate) {
            params.push(options.startDate);
        ...
        const historyResult = await database_1.pool.query(historyQuery, params);
        // Get statistics
        const statsQuery = `
            SELECT 
                COUNT(*) as total_attendance,
            ...
            WHERE a.church_id = $1 AND a.member_id = $2
        `;
        const statsResult = await database_1.pool.query(statsQuery, [churchId, memberId]);
        ...
        `;
        const rateResult = await database_1.pool.query(rateQuery, [churchId, memberId]);
        ...
        `;
        const absenceResult = await database_1.pool.query(absenceQuery, [churchId, memberId]);
    ...
    async getInactiveMembers(churchId, days) {
        const query = `
