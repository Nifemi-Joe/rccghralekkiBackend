...
class AuditLogRepository {
    ...
    async getEntityHistory(churchId, entityType, entityId) {
        const query = `
            ...
            LIMIT 100
        `;
        const result = await database_1.pool.query(query, [churchId, entityType, entityId]);
        return result.rows;
    }
    async deleteOld(churchId, daysOld = 90) {
        const result = await database_1.pool.query(`DELETE FROM audit_logs 
