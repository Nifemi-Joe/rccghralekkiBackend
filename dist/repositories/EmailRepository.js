...
class EmailRepository {
    ...
    async getDefaultConfiguration(churchId) {
        const query = `
            SELECT * FROM email_configurations 
    ...
    async setDefaultConfiguration(churchId, configId) {
        ...
        try {
            ...
            await client.query('UPDATE email_configurations SET is_default = false WHERE church_id = $1', [churchId]);
            await client.query('UPDATE email_configurations SET is_default = true WHERE id = $1 AND church_id = $2', [configId, churchId]);
    ...
    async deleteConfiguration(churchId, configId) {
        ...
        const result = await database_1.pool.query(query, [configId, churchId]);
    ...
    // ATTACHMENTS
    // ============================================================================
    async addAttachment(campaignId, data) {
        const query = `
    ...
    async getAttachments(campaignId) {
    ...
    async deleteAttachment(attachmentId) {
        ...
        const result = await database_1.pool.query(query, [attachmentId]);
        return (result.rowCount ?? 0) > 0;
    }
    // ============================================================================
    // EMAILS
    // ============================================================================
    async createEmail(churchId, data, createdBy) {
        const query = `
            INSERT INTO emails 
    ...
    async createEmails(churchId, emails, createdBy) {
        if (emails.length === 0)
            return [];
        const values = [];
        const placeholders = [];
        ...
        emails.forEach((e) => {
            ...
            values.push(churchId, e.campaignId || null, e.memberId || null, e.toEmail, e.toName || null, e.subject, e.htmlContent, e.textContent || null, createdBy || null);
            paramIndex += 9;
        });
        const query = `
            INSERT INTO emails 
    ...
    async updateEmailStatus(emailId, status, externalId, errorMessage) {
    ...
    async trackClick(emailId, url, ipAddress, userAgent) {
        const query = `
            ...
            RETURNING *
        `;
        const { rows } = await database_1.pool.query(query, [emailId, url, ipAddress, userAgent]);
        // Update email click count
        await this.updateEmailStatus(emailId, 'clicked');
        return rows[0];
    }
    async getClicksByEmail(emailId) {
        const query = `
            SELECT * FROM email_clicks 
            WHERE email_id = $1
            ORDER BY clicked_at DESC
        `;
        const { rows } = await database_1.pool.query(query, [emailId]);
        return rows;
    }
    // ============================================================================
    // STATISTICS
