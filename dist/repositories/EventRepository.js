...
class EventRepository {
    ...
    async findById(id, churchId, includeOfferings = true) {
    ...
    async findByQRCode(qrCode) {
        ...
        const result = await database_1.pool.query(query, [qrCode]);
        if (!result.rows[0])
            return null;
        const event = this.mapToEvent(result.rows[0]);
    ...
    async findAll(filters) {
        const { churchId, search, eventType, locationType, startDate, endDate, isActive, isPaid, isFeatured, groupId, page = 1, limit = 20, sortBy = 'start_date', sortOrder = 'asc' } = filters;
        let query = `
            SELECT e.*,
                   -- Offering totals
                   COALESCE((
        ...
        const [eventsResult, countResult] = await Promise.all([
            database_1.pool.query(query, params),
            database_1.pool.query(countQuery, params.slice(0, -2))
        ]);
    ...
    async getEventOfferings(eventId, churchId) {
        try {
            ...
            `;
            const detailsResult = await database_1.pool.query(detailsQuery, [eventId, churchId]);
            // Get summary
            const summaryQuery = `
                SELECT 
            ...
            `;
            const summaryResult = await database_1.pool.query(summaryQuery, [eventId, churchId]);
    ...
    async delete(id, churchId) {
        await database_1.pool.query('DELETE FROM events WHERE id = $1 AND church_id = $2', [id, churchId]);
    }
    async getStatistics(churchId) {
        ...
        `;
        try {
            const [statsResult, registrationsResult, attendanceResult, revenueResult, topEventsResult, byTypeResult, registrationsByMonthResult] = await Promise.all([
                database_1.pool.query(query, [churchId]),
                database_1.pool.query(registrationsQuery, [churchId]),
                database_1.pool.query(attendanceQuery, [churchId]),
                database_1.pool.query(revenueQuery, [churchId]),
                database_1.pool.query(topEventsQuery, [churchId]),
                database_1.pool.query(byTypeQuery, [churchId]),
                database_1.pool.query(registrationsByMonthQuery, [churchId])
            ]);
    ...
    // EVENT INSTANCES
    // ============================================================================
    async createInstance(churchId, data) {
        const id = (0, uuid_1.v4)();
        const qrCode = (0, helpers_1.generateQRCode)();
        const query = `
    ...
    async findInstances(eventId, churchId, options) {
        let query = `
    ...
    async findInstanceById(instanceId, churchId) {
        const query = `
            SELECT
                ei.*,
        ...
        `;
        const result = await database_1.pool.query(query, [instanceId, churchId]);
        return result.rows[0] ? this.mapToEventInstanceWithDetails(result.rows[0]) : null;
    }
    async findInstanceByQRCode(qrCode) {
        const query = `
            SELECT
                ei.*,
                e.name as event_name,
    ...
    async updateInstance(instanceId, churchId, data) {
        const fields = [];
        const values = [];
        let paramCount = 0;
        const fieldMappings = {
    ...
    async updateInstanceAttendance(instanceId, churchId, data) {
        const query = `
            UPDATE event_instances
            SET
                total_attendance = $3,
    ...
    // REGISTRATIONS
    // ============================================================================
    async createRegistration(churchId, data) {
        const id = (0, uuid_1.v4)();
    ...
    async findRegistrations(eventId, options) {
        let query = `
            SELECT er.*,
                   CONCAT(m.first_name, ' ', m.last_name) as member_name
            FROM event_registrations er
    ...
    async findRegistrationById(registrationId) {
        const query = `
            ...
            WHERE er.id = $1
        `;
        const result = await database_1.pool.query(query, [registrationId]);
        return result.rows[0] ? this.mapToEventRegistration(result.rows[0]) : null;
    }
    async findRegistrationByQRCode(qrCode) {
        const query = `
            SELECT er.*, 
                   CONCAT(m.first_name, ' ', m.last_name) as member_name
            FROM event_registrations er
    ...
    async updateRegistrationPayment(registrationId, paymentData) {
        const query = `
            UPDATE event_registrations
            SET
                payment_status = $2,
    ...
    async checkIn(registrationId, checkedInBy) {
        const query = `
            UPDATE event_registrations 
            SET 
                checked_in = true,
            ...
            RETURNING *
        `;
        const result = await database_1.pool.query(query, [registrationId, checkedInBy || null]);
        return result.rows[0] ? this.mapToEventRegistration(result.rows[0]) : null;
    }
    // ============================================================================
    // TICKET TYPES
    ...
    async getTicketTypes(eventId) {
        ...
        `;
        const result = await database_1.pool.query(query, [eventId]);
        return result.rows.map(row => this.mapToTicketType(row));
    }
    // ============================================================================
    // SHARING
    // ============================================================================
    async createShare(churchId, eventId, data) {
        const id = (0, uuid_1.v4)();
        const query = `
    ...
    async getEventFinancials(eventId, churchId) {
    ...
    mapToEventInstanceWithDetails(row) {
        return {
