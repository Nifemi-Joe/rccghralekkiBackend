...
class FamilyRepository {
    ...
    async delete(id, churchId) {
      ...
      WHERE family_id = $1
    `, [id]);
        const result = await database_1.pool.query('DELETE FROM families WHERE id = $1 AND church_id = $2', [id, churchId]);
    ...
    async getFamilyMembers(familyId) {
      ...
      ORDER BY 
        ...
        m.first_name ASC
    `;
        const result = await database_1.pool.query(query, [familyId]);
    ...
    async addMember(familyId, data) {
      ...
      `;
            ...
            if (data.isHead) {
                await client.query('UPDATE families SET head_id = $1, updated_at = NOW() WHERE id = $2', [data.memberId, familyId]);
            }
    ...
    async removeMember(familyId, memberId) {
        ...
        try {
            ...
            const familyResult = await client.query('SELECT head_id FROM families WHERE id = $1', [familyId]);
            if (familyResult.rows[0]?.head_id === memberId) {
                // Clear head_id from family
                await client.query('UPDATE families SET head_id = NULL, updated_at = NOW() WHERE id = $1', [familyId]);
            }
    ...
    async updateMember(familyId, memberId, data) {
        ...
        `, values);
            ...
            // Handle head status
            if (data.isHead === true) {
                await client.query('UPDATE families SET head_id = $1, updated_at = NOW() WHERE id = $2', [memberId, familyId]);
            }
            else if (data.isHead === false) {
                // Only clear if this member is currently the head
                await client.query('UPDATE families SET head_id = NULL, updated_at = NOW() WHERE id = $1 AND head_id = $2', [familyId, memberId]);
            }
            await client.query('COMMIT');
            return true;
        }
