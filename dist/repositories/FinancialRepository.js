...
class FinancialRepository {
    ...
    async createAccount(churchId, data) {
        ...
        try {
            ...
            logger_1.default.error('Error in FinancialRepository.createAccount:', error);
    ...
    async findAllAccounts(churchId) {
        const client = await database_1.pool.connect();
        try {
            const query = `
                SELECT a.*,
            ...
            logger_1.default.error('Error in FinancialRepository.findAllAccounts:', error);
            throw new AppError_1.AppError('Failed to fetch accounts', 500);
        }
        finally {
            client.release();
        }
    }
    async findAccountById(churchId, accountId) {
        const client = await database_1.pool.connect();
        try {
            ...
            logger_1.default.error('Error in FinancialRepository.findAccountById:', error);
            throw new AppError_1.AppError('Failed to fetch account', 500);
        }
        finally {
            client.release();
        }
    ...
    async updateAccount(churchId, accountId, data) {
        ...
        try {
            ...
            logger_1.default.error('Error in FinancialRepository.updateAccount:', error);
            throw new AppError_1.AppError('Failed to update account', 500);
        }
        finally {
            client.release();
        }
    ...
    async deleteAccount(churchId, accountId) {
        ...
        try {
            ...
            if (parseInt(checkResult.rows[0].count) > 0) {
                throw new AppError_1.AppError('Cannot delete account with existing transactions', 400);
            }
            ...
            if (error instanceof AppError_1.AppError)
                throw error;
            logger_1.default.error('Error in FinancialRepository.deleteAccount:', error);
            throw new AppError_1.AppError('Failed to delete account', 500);
        }
        finally {
            client.release();
        }
    ...
    async createTransaction(churchId, userId, data) {
        ...
        try {
            ...
            await client.query(`UPDATE accounts SET balance = balance + $1, updated_at = NOW() WHERE id = $2`, [amount, data.account_id]);
            ...
            logger_1.default.error('Error in FinancialRepository.createTransaction:', error);
            throw new AppError_1.AppError('Failed to create transaction', 500);
        }
        finally {
            client.release();
        }
    ...
    async createExpense(churchId, userId, data) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            const query = `
            ...
            await client.query('ROLLBACK');
            logger_1.default.error('Error in FinancialRepository.createExpense:', error);
            throw new AppError_1.AppError('Failed to create expense', 500);
        }
        finally {
            client.release();
        }
    }
    async approveExpense(churchId, transactionId, approvedBy, status, rejectionReason) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            // Get the expense with lock
            const expenseResult = await client.query(`SELECT * FROM transactions
                 WHERE id = $1 AND church_id = $2 AND transaction_type = 'expense'
                     FOR UPDATE`, [transactionId, churchId]);
            if (expenseResult.rows.length === 0) {
                throw new AppError_1.AppError('Expense not found', 404);
            }
            const expense = expenseResult.rows[0];
            if (expense.approval_status !== 'pending') {
                throw new AppError_1.AppError(`Expense is already ${expense.approval_status}`, 400);
            }
            // Update expense approval status
            const updateResult = await client.query(`UPDATE transactions
                 ...
                 WHERE id = $4 AND church_id = $5
                     RETURNING *`, [status, approvedBy, rejectionReason || null, transactionId, churchId]);
            // Update account balance only if approved
            if (status === 'approved') {
                await client.query(`UPDATE accounts
                     SET balance = balance + $1,
                         updated_at = NOW()
                     WHERE id = $2 AND church_id = $3`, [expense.amount, expense.account_id, churchId]);
            }
            await client.query('COMMIT');
            ...
            if (error instanceof AppError_1.AppError)
                throw error;
            logger_1.default.error('Error in FinancialRepository.approveExpense:', error);
            throw new AppError_1.AppError('Failed to approve expense', 500);
        }
        finally {
            client.release();
        }
    ...
    async getExpenseApprovalSummary(churchId) {
        ...
        try {
            const result = await client.query(`SELECT
                 ...
                 GROUP BY approval_status`, [churchId]);
            const summary = {
                pendingCount: 0,
                pendingAmount: 0,
                approvedCount: 0,
            ...
            logger_1.default.error('Error in FinancialRepository.getExpenseApprovalSummary:', error);
            throw new AppError_1.AppError('Failed to get expense approval summary', 500);
        }
        finally {
            client.release();
        }
    ...
    // ============ BATCH OFFERING ============
    async createBatchOffering(churchId, userId, data) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            const transactions = [];
            ...
            for (const item of data.items) {
                ...
                await client.query(`UPDATE accounts SET balance = balance + $1, updated_at = NOW() WHERE id = $2`, [Math.abs(item.amount), item.account_id]);
            }
            if (transactions.length === 0) {
                await client.query('ROLLBACK');
                throw new AppError_1.AppError('No valid offering items to process', 400);
            }
            await client.query('COMMIT');
            logger_1.default.info(`Batch offering recorded: ${transactions.length} transactions, total: ${totalAmount}`);
            ...
            if (error instanceof AppError_1.AppError)
                throw error;
            logger_1.default.error('Error in FinancialRepository.createBatchOffering:', error);
            throw new AppError_1.AppError('Failed to create batch offering', 500);
        }
        finally {
            client.release();
        }
    }
    async findAllTransactions(churchId, filters = {}) {
        const client = await database_1.pool.connect();
        try {
            ...
            logger_1.default.error('Error in FinancialRepository.findAllTransactions:', error);
            throw new AppError_1.AppError('Failed to fetch transactions', 500);
        }
        finally {
            client.release();
        }
    }
    async findTransactionById(churchId, transactionId) {
        const client = await database_1.pool.connect();
        try {
            const query = `
                SELECT t.*,
            ...
            logger_1.default.error('Error in FinancialRepository.findTransactionById:', error);
            throw new AppError_1.AppError('Failed to fetch transaction', 500);
        }
        finally {
            client.release();
        }
    ...
    async deleteTransaction(churchId, transactionId) {
        ...
        try {
            ...
            if (transaction.transaction_type !== 'expense' || transaction.approval_status === 'approved') {
                await client.query(`UPDATE accounts SET balance = balance - $1, updated_at = NOW() WHERE id = $2`, [transaction.amount, transaction.account_id]);
            }
            ...
            await client.query('ROLLBACK');
            logger_1.default.error('Error in FinancialRepository.deleteTransaction:', error);
            throw new AppError_1.AppError('Failed to delete transaction', 500);
        }
        finally {
            client.release();
        }
    ...
    // ============ EVENT FINANCIALS ============
    async findTransactionsByEventId(churchId, eventId, type) {
        const client = await database_1.pool.connect();
        try {
            let typeFilter = '';
            if (type === 'income') {
            ...
            logger_1.default.error('Error in FinancialRepository.findTransactionsByEventId:', error);
            throw new AppError_1.AppError('Failed to fetch event transactions', 500);
        }
        finally {
            client.release();
        }
    ...
    async getEventFinancialSummary(churchId, eventId) {
        ...
        try {
            ...
            logger_1.default.error('Error in FinancialRepository.getEventFinancialSummary:', error);
            throw new AppError_1.AppError('Failed to fetch event financial summary', 500);
        }
        finally {
            client.release();
        }
    ...
    async getMonthlyTrend(churchId, filters = {}) {
        ...
        try {
            ...
            logger_1.default.error('Error in FinancialRepository.getMonthlyTrend:', error);
            throw new AppError_1.AppError('Failed to fetch monthly trend', 500);
        }
        finally {
            client.release();
        }
    ...
    async getFinancialSummary(churchId, startDate, endDate, includeTrend = true, trendMonths = 6) {
        ...
        try {
            ...
            logger_1.default.error('Error in FinancialRepository.getFinancialSummary:', error);
            throw new AppError_1.AppError('Failed to fetch financial summary', 500);
        }
        finally {
            try {
                client.release();
    ...
    async createExpenseCategory(churchId, data) {
        ...
        try {
            ...
            logger_1.default.error('Error in FinancialRepository.createExpenseCategory:', error);
            throw new AppError_1.AppError('Failed to create expense category', 500);
        }
        finally {
            client.release();
        }
    }
    async findAllExpenseCategories(churchId) {
        const client = await database_1.pool.connect();
        try {
            const query = `
                SELECT ec.*,
            ...
            logger_1.default.error('Error in FinancialRepository.findAllExpenseCategories:', error);
            throw new AppError_1.AppError('Failed to fetch expense categories', 500);
        }
        finally {
            client.release();
        }
    }
    async findExpenseCategoryById(churchId, categoryId) {
        const client = await database_1.pool.connect();
        try {
            const query = `
            ...
            logger_1.default.error('Error in FinancialRepository.findExpenseCategoryById:', error);
            throw new AppError_1.AppError('Failed to fetch expense category', 500);
        }
        finally {
            client.release();
        }
    ...
    async updateExpenseCategory(churchId, categoryId, data) {
        ...
        try {
            ...
            logger_1.default.error('Error in FinancialRepository.updateExpenseCategory:', error);
            throw new AppError_1.AppError('Failed to update expense category', 500);
        }
        finally {
            client.release();
        }
    ...
    async deleteExpenseCategory(churchId, categoryId) {
        ...
        try {
            ...
            logger_1.default.error('Error in FinancialRepository.deleteExpenseCategory:', error);
            throw new AppError_1.AppError('Failed to delete expense category', 500);
        }
        finally {
            client.release();
        }
