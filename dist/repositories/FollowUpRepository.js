...
class FollowUpRepository {
    ...
    // DEPARTMENT
    // ============================================================================
    async getOrCreateDepartment(churchId) {
        const client = await database_1.pool.connect();
        try {
            let result = await client.query('SELECT * FROM follow_up_departments WHERE church_id = $1', [churchId]);
            ...
            result = await client.query(`INSERT INTO follow_up_departments (church_id, name, description, settings)
                 VALUES ($1, 'Follow Up Department', 'Department responsible for following up with first-time visitors', $2)
                     RETURNING *`, [churchId, JSON.stringify(defaultSettings)]);
            return this.mapDepartment(result.rows[0]);
        }
        finally {
            client.release();
    ...
    async updateDepartmentSettings(churchId, settings) {
        ...
        try {
            ...
            const result = await client.query(`UPDATE follow_up_departments
                 SET settings = $1, updated_at = NOW()
                 WHERE church_id = $2
                     RETURNING *`, [JSON.stringify(newSettings), churchId]);
            return this.mapDepartment(result.rows[0]);
        }
        finally {
            client.release();
    ...
    async getMembers(churchId, filters) {
        ...
        return rows.map((row) => this.mapMemberWithStats(row));
    }
    async getMemberById(churchId, followUpMemberId) {
        const query = `
            SELECT
                fm.*,
                m.first_name,
        ...
        `;
        const { rows } = await database_1.pool.query(query, [followUpMemberId, churchId]);
        if (rows.length === 0)
            return null;
        return this.mapMemberWithStats(rows[0]);
    }
    async addMember(churchId, data, userId) {
        const client = await database_1.pool.connect();
        try {
            const department = await this.getOrCreateDepartment(churchId);
            const memberCheck = await client.query('SELECT id FROM members WHERE id = $1 AND church_id = $2', [data.memberId, churchId]);
            if (memberCheck.rows.length === 0) {
                throw new Error('Member not found in this church');
            }
            const existingCheck = await client.query('SELECT id FROM follow_up_members WHERE department_id = $1 AND member_id = $2', [department.id, data.memberId]);
            if (existingCheck.rows.length > 0) {
                throw new Error('Member is already in the follow-up department');
            }
            const result = await client.query(`INSERT INTO follow_up_members (department_id, member_id, church_id, role)
                 VALUES ($1, $2, $3, $4)
                     RETURNING *`, [department.id, data.memberId, churchId, data.role || 'member']);
            const memberResult = await client.query('SELECT first_name, last_name, email, phone, profile_image_url FROM members WHERE id = $1', [data.memberId]);
    ...
    async updateMember(churchId, followUpMemberId, data) {
        ...
        await database_1.pool.query(`UPDATE follow_up_members
             SET ${setClauses.join(', ')}
             WHERE id = $${paramIndex++} AND church_id = $${paramIndex}`, params);
        return this.getMemberById(churchId, followUpMemberId);
    }
    async removeMember(churchId, followUpMemberId) {
        ...
        try {
            ...
            await client.query(`UPDATE follow_up_assigned_members
                 ...
                 WHERE follow_up_member_id = $1`, [followUpMemberId]);
            const result = await client.query('DELETE FROM follow_up_members WHERE id = $1 AND church_id = $2 RETURNING id', [followUpMemberId, churchId]);
    ...
    async getAssignments(churchId, filters) {
        ...
        let paramIndex = 2;
        if (filters.search) {
            whereConditions.push(`(ft.first_name ILIKE $${paramIndex} OR ft.last_name ILIKE $${paramIndex})`);
            params.push(`%${filters.search}%`);
            paramIndex++;
        }
        if (filters.status) {
        ...
        if (filters.assignedMemberId) {
            whereConditions.push(`EXISTS (SELECT 1 FROM follow_up_assigned_members fam ` +
    ...
    async getAssignmentById(churchId, assignmentId) {
        const result = await database_1.pool.query(`SELECT
             ...
             FROM follow_up_assignments fa
                      JOIN first_timers ft ON fa.first_timer_id = ft.id
             WHERE fa.id = $1 AND fa.church_id = $2`, [assignmentId, churchId]);
        if (result.rows.length === 0)
            return null;
    ...
    async getAssignmentByFirstTimer(churchId, firstTimerId) {
        const result = await database_1.pool.query(`SELECT
             ...
             ORDER BY fa.created_at DESC
                 LIMIT 1`, [firstTimerId, churchId]);
        if (result.rows.length === 0)
            return null;
        const assignedMembers = await this.getAssignedMembers(result.rows[0].id);
        return this.mapAssignment(result.rows[0], assignedMembers);
    }
    async createAssignment(churchId, data, userId) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            const ftCheck = await client.query('SELECT id, status FROM first_timers WHERE id = $1 AND church_id = $2', [data.firstTimerId, churchId]);
            if (ftCheck.rows.length === 0) {
                throw new Error('First timer not found');
            }
            const existingCheck = await client.query(`SELECT id FROM follow_up_assignments
                 WHERE first_timer_id = $1 AND church_id = $2 AND status = 'active'`, [data.firstTimerId, churchId]);
            if (existingCheck.rows.length > 0) {
                throw new Error('An active assignment already exists for this first timer');
            }
            const assignmentResult = await client.query(`INSERT INTO follow_up_assignments
            ...
            for (const memberId of data.assignedMemberIds) {
                await client.query(`INSERT INTO follow_up_assigned_members
                     ...
                     VALUES ($1, $2, $3)`, [assignmentId, memberId, memberId === primaryMemberId]);
            }
            await client.query(`UPDATE first_timers SET follow_up_status = 'scheduled', updated_at = NOW()
                 WHERE id = $1`, [data.firstTimerId]);
            await client.query('COMMIT');
    ...
    async updateAssignment(churchId, assignmentId, data) {
        ...
        await database_1.pool.query(`UPDATE follow_up_assignments
             SET ${setClauses.join(', ')}
             WHERE id = $${paramIndex++} AND church_id = $${paramIndex}`, params);
        return this.getAssignmentById(churchId, assignmentId);
    }
    async completeAssignment(churchId, assignmentId, notes) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            const result = await client.query(`UPDATE follow_up_assignments
                 SET status = 'completed', completed_at = NOW(), completion_notes = $1, updated_at = NOW()
                 WHERE id = $2 AND church_id = $3
                     RETURNING first_timer_id`, [notes || null, assignmentId, churchId]);
            if (result.rows.length === 0) {
                await client.query('ROLLBACK');
                return null;
            }
            ...
            await client.query(`UPDATE first_timers
                 ...
                 WHERE id = $1`, [result.rows[0].first_timer_id]);
            await client.query('COMMIT');
            return this.getAssignmentById(churchId, assignmentId);
        }
        catch (error) {
    ...
    async getAssignedMembers(assignmentId) {
        const result = await database_1.pool.query(`SELECT
                 fam.*,
                 m.first_name,
                 m.last_name,
             ...
             ORDER BY fam.is_primary DESC, fam.assigned_at`, [assignmentId]);
    ...
    async getActivities(churchId, assignmentId, filters) {
        ...
        const params = [assignmentId, churchId];
        let paramIndex = 3;
        if (filters.channel) {
        ...
        const whereClause = whereConditions.join(' AND ');
        const countResult = await database_1.pool.query(`SELECT COUNT(*) as total FROM follow_up_activities fua WHERE ${whereClause}`, params);
        const total = parseInt(countResult.rows[0].total);
        const page = filters.page || 1;
        const limit = filters.limit || 20;
        const offset = (page - 1) * limit;
    ...
    async createActivity(churchId, userId, data) {
        const result = await database_1.pool.query(`INSERT INTO follow_up_activities
        ...
        await database_1.pool.query(`UPDATE first_timers
    ...
    async createMessageActivity(churchId, userId, data) {
        const result = await database_1.pool.query(`INSERT INTO follow_up_activities
        ...
        await database_1.pool.query(`UPDATE first_timers
             SET follow_up_status = 'contacted', last_contact_date = NOW(), updated_at = NOW()
             WHERE id = $1 AND follow_up_status IN ('pending', 'scheduled')`, [data.firstTimerId]);
        const userResult = await database_1.pool.query('SELECT first_name, last_name FROM users WHERE id = $1', [userId]);
        return this.mapActivity({
            ...result.rows[0],
            performer_first_name: userResult.rows[0]?.first_name,
    ...
    async updateActivityDeliveryStatus(activityId, deliveryStatus, externalMessageId) {
        await database_1.pool.query(`UPDATE follow_up_activities
             SET delivery_status = $1, external_message_id = COALESCE($2, external_message_id), updated_at = NOW()
             WHERE id = $3`, [deliveryStatus, externalMessageId, activityId]);
    }
    async recordResponse(churchId, activityId, response) {
        const result = await database_1.pool.query(`UPDATE follow_up_activities
             SET response = $1, response_at = NOW(), updated_at = NOW()
             WHERE id = $2 AND church_id = $3
                 RETURNING *`, [response, activityId, churchId]);
        if (result.rows.length === 0)
            return null;
        const userResult = await database_1.pool.query('SELECT first_name, last_name FROM users WHERE id = $1', [result.rows[0].performed_by]);
        return this.mapActivity({
            ...result.rows[0],
            performer_first_name: userResult.rows[0]?.first_name,
            performer_last_name: userResult.rows[0]?.last_name,
    ...
    async createTemplate(churchId, userId, data) {
        ...
        const result = await database_1.pool.query(`INSERT INTO follow_up_templates
    ...
    async updateTemplate(churchId, templateId, data) {
        ...
        if (setClauses.length === 0) {
            const result = await database_1.pool.query('SELECT * FROM follow_up_templates WHERE id = $1 AND church_id = $2', [templateId, churchId]);
            return result.rows[0] ? this.mapTemplate(result.rows[0]) : null;
        }
        setClauses.push('updated_at = NOW()');
        params.push(templateId, churchId);
        const result = await database_1.pool.query(`UPDATE follow_up_templates
             SET ${setClauses.join(', ')}
    ...
    async deleteTemplate(churchId, templateId) {
        const result = await database_1.pool.query('DELETE FROM follow_up_templates WHERE id = $1 AND church_id = $2 RETURNING id', [templateId, churchId]);
        return result.rowCount !== null && result.rowCount > 0;
    }
    // ============================================================================
    // STATISTICS
    ...
    async getStatistics(churchId) {
        const countsResult = await database_1.pool.query(`SELECT
                 COUNT(*) as total,
        ...
        const avgResult = await database_1.pool.query(`SELECT AVG(EXTRACT(EPOCH FROM (completed_at - created_at)) / 86400) as avg_days
             FROM follow_up_assignments
             WHERE church_id = $1 AND status = 'completed' AND completed_at IS NOT NULL`, [churchId]);
        const channelResult = await database_1.pool.query(`SELECT channel, COUNT(*) as count
             FROM follow_up_activities
             WHERE church_id = $1
             GROUP BY channel`, [churchId]);
        const activitiesByChannel = {};
        let totalActivities = 0;
        channelResult.rows.forEach((row) => {
            activitiesByChannel[row.channel] = parseInt(row.count);
            totalActivities += parseInt(row.count);
        });
        const performanceResult = await database_1.pool.query(`SELECT
                 fm.id as member_id,
             ...
             GROUP BY fm.id, m.first_name, m.last_name`, [churchId]);
        const memberPerformance = performanceResult.rows.map((row) => {
            ...
            return {
                ...
                successRate: active + completed > 0
                    ? Math.round((completed / (active + completed)) * 100)
                    : 0,
            };
        });
        const trendResult = await database_1.pool.query(`SELECT
             ...
             ORDER BY DATE_TRUNC('week', created_at)`, [churchId]);
        ...
        return {
            ...
            totalActivities,
            activitiesByChannel: activitiesByChannel,
    ...
    async getUnassignedFirstTimers(churchId) {
        const result = await database_1.pool.query(`SELECT ft.*
             ...
             ORDER BY ft.first_visit_date DESC`, [churchId]);
        return result.rows;
    }
    // ============================================================================
    // MAPPERS
    ...
    mapDepartment(row) {
        ...
        return {
            ...
            settings: {
                ...
                defaultMessageTemplates: settings.defaultMessageTemplates ??
    ...
    mapAssignment(row, assignedMembers) {
        return {
            id: row.id,
            churchId: row.church_id,
