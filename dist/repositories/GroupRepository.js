...
class GroupRepository {
    ...
    async findById(churchId, groupId) {
        const query = `
      SELECT 
        g.*,
        gt.name as group_type_name,
    ...
    async update(churchId, groupId, data) {
        const fieldMapping = {
            name: 'name',
    ...
    `;
        const statsResult = await database_1.pool.query(statsQuery, [churchId]);
    ...
    `;
        const meetingsResult = await database_1.pool.query(meetingsQuery, [churchId]);
      ...
      ORDER BY count DESC
    `;
        const typeResult = await database_1.pool.query(typeQuery, [churchId]);
        return {
    ...
    async updateMemberRole(groupId, memberId, role) {
      ...
      RETURNING *
    `;
        await database_1.pool.query(query, [groupId, memberId, role]);
        return this.getMemberById(groupId, memberId);
    }
    async getMemberById(groupId, memberId) {
        const query = `
      SELECT 
        gm.*,
    ...
    async isMember(groupId, memberId) {
    ...
    `;
        ...
        const result = await database_1.pool.query(query, values);
        return this.getMeetingById(result.rows[0].id, churchId);
    }
    ...
    `;
        const result = await database_1.pool.query(query, [meetingId, churchId]);
        if (!result.rows[0])
            return null;
        const row = result.rows[0];
        return {
    ...
    async getGroupMeetings(groupId, options) {
    ...
    async markMeetingShared(meetingId, shareType) {
        const fieldMap = {
        ...
        await database_1.pool.query(query, [meetingId]);
    }
    // ============================================================================
    // GROUP TYPES
    // ============================================================================
    ...
    async deleteGroupType(churchId, typeId) {
        ...
        const result = await database_1.pool.query(query, [typeId, churchId]);
        return (result.rowCount ?? 0) > 0;
    }
}
