...
class MemberRepository {
    ...
    async create(data, options) {
        ...
        try {
            ...
            logger_1.default.error('Error creating member:', error);
            throw error;
        }
        finally {
            client.release();
     ...
     */
    async findByProfileToken(token) {
        const query = `
            SELECT * FROM members
     ...
     */
    async updateViaToken(token, data, ipAddress, userAgent) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            // Verify token
            ...
            updates.push(`updated_at = NOW()`, `profile_completed_at = NOW()`, `profile_update_token = NULL`, `profile_update_token_expires_at = NULL`);
            const query = `
                UPDATE members
                SET ${updates.join(', ')}
     ...
     */
    async logProfileLinkSent(memberId, churchId, channels, actorId, ipAddress, userAgent) {
        await this.createAuditLog({
            churchId,
            entityType: 'member',
            entityId: memberId,
    ...
    async getCelebrations(churchId, filters) {
        ...
        const [countResult, dataResult] = await Promise.all([
            database_1.pool.query(countQuery, [churchId, type, period, days]),
            database_1.pool.query(query + ` LIMIT $5 OFFSET $6`, [churchId, type, period, days, limit, offset])
        ]);
    ...
    async findAll(filters) {
        const { churchId, search, status, gender, maritalStatus, familyId, hasFamily, hasUserAccount, profileCompleted, page = 1, limit = 20, sortBy = 'created_at', sortOrder = 'desc', } = filters;
     ...
     */
    async getMembersByGroup(churchId, groupId) {
        try {
            const query = `
            SELECT m.* 
            FROM members m
        ...
        `;
            const result = await database_1.pool.query(query, [groupId, churchId]);
            return result.rows;
        }
        catch (error) {
            logger_1.default.error('Error in getMembersByGroup:', error);
            throw new AppError_1.AppError('Failed to get members by group', 500);
        }
    }
    /**
     * Search members by name with fuzzy matching
     */
    async searchByName(churchId, firstName, lastName, limit = 5) {
        try {
            ...
            const exactMatchQuery = `
                ...
                LIMIT $4
            `;
            const exactResult = await database_1.pool.query(exactMatchQuery, [churchId, firstName, lastName, limit]);
            ...
            const fuzzyMatchQuery = `
                ...
                LIMIT $8
            `;
            const fuzzyResult = await database_1.pool.query(fuzzyMatchQuery, [
                churchId,
                `%${firstName}%`,
                `%${lastName}%`,
                `%${lastName}%`,
            ...
            logger_1.default.error('Error in searchByName:', error);
            throw new AppError_1.AppError('Failed to search members by name', 500);
        }
    }
    ...
    async findByUserId(userId, churchId) {
        const client = await database_1.pool.connect();
        try {
            const query = `
                SELECT * FROM members
            ...
            logger_1.default.error('Error in MemberRepository.findByUserId:', error);
            throw new AppError_1.AppError('Failed to fetch member by user ID', 500);
        }
        finally {
            client.release();
        }
    ...
    async update(id, churchId, data, actorId, ipAddress, userAgent) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            ...
            for (const [key, value] of Object.entries(processedData)) {
                if (value !== undefined && fieldMapping[key]) {
                    const dbField = fieldMapping[key];
                    const oldValue = currentMember[dbField];
                    if (oldValue !== value) {
                        changes[dbField] = { old: oldValue, new: value };
                    }
            ...
            await client.query('ROLLBACK');
            logger_1.default.error('Error updating member:', error);
            throw error;
        }
        finally {
            client.release();
    ...
    async delete(id, churchId, actorId, ipAddress, userAgent) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            ...
            await client.query('ROLLBACK');
            logger_1.default.error('Error deleting member:', error);
            throw error;
        }
        finally {
            client.release();
    ...
    async getStatistics(churchId) {
        ...
        `;
        const statusResult = await database_1.pool.query(statusQuery, [churchId]);
        ...
        const genderQuery = `
            ...
            WHERE church_id = $1 AND deleted_at IS NULL
        `;
        const genderResult = await database_1.pool.query(genderQuery, [churchId]);
        ...
        const maritalQuery = `
            ...
            WHERE church_id = $1 AND deleted_at IS NULL
        `;
        const maritalResult = await database_1.pool.query(maritalQuery, [churchId]);
        ...
        const growthQuery = `
            ...
            WHERE church_id = $1 AND deleted_at IS NULL
        `;
        const growthResult = await database_1.pool.query(growthQuery, [churchId]);
        ...
        `;
        const birthdayResult = await database_1.pool.query(birthdayQuery, [churchId]);
        ...
        `;
        const anniversaryResult = await database_1.pool.query(anniversaryQuery, [churchId]);
        const familyQuery = `
            SELECT
            ...
            WHERE church_id = $1 AND deleted_at IS NULL
        `;
        const familyResult = await database_1.pool.query(familyQuery, [churchId]);
        ...
        const userAccountQuery = `
            ...
            WHERE church_id = $1 AND deleted_at IS NULL
        `;
        const userAccountResult = await database_1.pool.query(userAccountQuery, [churchId]);
        ...
        const profileQuery = `
            ...
            WHERE church_id = $1 AND deleted_at IS NULL
        `;
        const profileResult = await database_1.pool.query(profileQuery, [churchId]);
    ...
    async search(query, churchId, limit = 10) {
        const searchQuery = `
            SELECT * FROM members
            WHERE church_id = $1
              AND deleted_at IS NULL
            ...
            ORDER BY first_name, last_name
                LIMIT $3
        `;
        const result = await database_1.pool.query(searchQuery, [churchId, `%${query}%`, limit]);
        return result.rows;
    }
    async getUpcomingAnniversaries(churchId, days = 30) {
        const query = `
            SELECT * FROM members
            WHERE church_id = $1
              AND deleted_at IS NULL
    ...
    async linkUserToMember(memberId, userId, churchId) {
        const query = `
            UPDATE members
            ...
            WHERE id = $2 AND church_id = $3 AND deleted_at IS NULL
                RETURNING *
        `;
        const result = await database_1.pool.query(query, [userId, memberId, churchId]);
        return result.rows[0] || null;
    }
    async unlinkUserFromMember(memberId, churchId) {
        const query = `
            UPDATE members
            ...
            WHERE id = $1 AND church_id = $2 AND deleted_at IS NULL
                RETURNING *
        `;
        const result = await database_1.pool.query(query, [memberId, churchId]);
        return result.rows[0] || null;
    }
    /**
    ...
    async getAuditLogs(memberId, churchId, options) {
        ...
        `;
        const [countResult, dataResult] = await Promise.all([
            database_1.pool.query(countQuery, [churchId, memberId]),
            database_1.pool.query(dataQuery, [churchId, memberId, limit, offset])
        ]);
        const total = parseInt(countResult.rows[0].total, 10);
        return {
