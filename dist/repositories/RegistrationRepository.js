...
class RegistrationRepository {
    ...
    // ============================================================================
    async searchRegistrants(churchId, query, limit = 20) {
        ...
        const [memberResult, firstTimerResult] = await Promise.all([
            database_1.pool.query(memberQuery, [churchId, searchPattern, exactPattern, halfLimit]),
            database_1.pool.query(firstTimerQuery, [churchId, searchPattern, exactPattern, halfLimit]),
        ]);
        const results = [];
    ...
    // ============================================================================
    async getEventsForRegistration(churchId) {
        const query = `
            SELECT 
                id,
                name,
    ...
    async getEventWithInstances(eventId, churchId) {
        const eventQuery = `
            SELECT 
        ...
        `;
        const eventResult = await database_1.pool.query(eventQuery, [eventId, churchId]);
        ...
        const instancesQuery = `
            ...
            LIMIT 10
        `;
        const instancesResult = await database_1.pool.query(instancesQuery, [eventId, churchId]);
        return {
            event: {
                id: event.id,
                name: event.name,
    ...
    // ============================================================================
    async createFirstTimer(churchId, data) {
    ...
    // CREATE REGISTRATION
    // ============================================================================
    async createRegistration(churchId, eventId, eventInstanceId, registrantId, registrantSource, options) {
        ...
        try {
            ...
            if (existingResult.rows[0]) {
                ...
                qrCode = (0, helpers_1.generateQRCode)();
                const insertQuery = `
    ...
    async checkIn(churchId, data, checkedInBy) {
        ...
        const registration = await this.createRegistration(churchId, data.eventId, data.eventInstanceId, registrantId, registrantSource, {
            checkedIn: true,
    ...
    // REMOVE CHECK-IN
    // ============================================================================
    async removeCheckIn(registrationId, churchId) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
    ...
    // GET EVENT ATTENDEES
    // ============================================================================
    async getEventAttendees(eventId, churchId, instanceId) {
        const query = `
            SELECT 
                er.id,
                er.member_id,
        ...
        return result.rows.map(row => {
            ...
            return {
                ...
                registrantSource: isMember ? 'member' : 'first_timer',
    ...
    // ============================================================================
    async getAttendanceStats(eventId, churchId, instanceId) {
        const query = `
            SELECT 
    ...
    // ============================================================================
    async getRecentCheckIns(eventId, churchId, limit = 10) {
        const query = `
            SELECT 
                er.id,
            ...
            LIMIT $3
        `;
        const result = await database_1.pool.query(query, [eventId, churchId, limit]);
        return result.rows.map(row => ({
            id: row.id,
            name: row.name,
            time: row.time,
