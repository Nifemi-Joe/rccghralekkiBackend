...
class ReportRepository {
        ...
        try {
            const query = `
                    ...
                    WHERE church_id = $1
                     ...
                     group_stats AS (
                         SELECT
                             ...
                             COUNT(*) FILTER (WHERE deleted_at IS NULL) as active_groups
                         FROM groups
                ...
                SELECT
                    ...
                    COALESCE(SUM(CASE WHEN amount > 0 AND transaction_date >= DATE_TRUNC('month', CURRENT_DATE) THEN amount ELSE 0 END), 0) as revenue_this_month,
                    COALESCE(SUM(CASE WHEN amount < 0 AND transaction_date >= DATE_TRUNC('month', CURRENT_DATE) THEN ABS(amount) ELSE 0 END), 0) as expenses_this_month
                FROM transactions
                WHERE church_id = $1
                    )
                SELECT
            ...
            logger_1.default.error('Error in ReportRepository.getDashboardStats:', error);
            throw new AppError_1.AppError('Failed to fetch dashboard stats', 500);
        }
        finally {
            client.release();
        }
    ...
    async getAttendanceTrends(churchId, filters) {
        ...
        try {
            ...
            let eventFilter = '';
            const params = [churchId, filters.startDate, filters.endDate];
            let paramIndex = 4;
            if (filters.eventId) {
            ...
            const query = `
                WITH attendance_data AS (
                    SELECT
                        ...
                        COALESCE(SUM(COALESCE(ei.member_attendance, 0)), 0) as member_attendance,
                        COALESCE(SUM(COALESCE(ei.guest_attendance, 0)), 0) as guest_attendance,
            ...
            logger_1.default.error('Error in ReportRepository.getAttendanceTrends:', error);
            throw new AppError_1.AppError('Failed to fetch attendance trends', 500);
        }
        finally {
            client.release();
        }
    ...
    async getMemberGrowth(churchId, filters) {
        ...
        try {
            ...
            logger_1.default.error('Error in ReportRepository.getMemberGrowth:', error);
            throw new AppError_1.AppError('Failed to fetch member growth', 500);
        }
        finally {
            client.release();
        }
    ...
    async getFirstTimerConversion(churchId, filters) {
        ...
        try {
            ...
            logger_1.default.error('Error in ReportRepository.getFirstTimerConversion:', error);
            throw new AppError_1.AppError('Failed to fetch first timer conversion', 500);
        }
        finally {
            client.release();
        }
    ...
    async getFinancialSummary(churchId, filters) {
        ...
        try {
            ...
            const query = `
                WITH financial_data AS (
                    SELECT
                        ...
                        COALESCE(SUM(CASE WHEN t.transaction_type = 'tithe' THEN t.amount ELSE 0 END), 0) as tithes,
                        COALESCE(SUM(CASE WHEN t.transaction_type = 'offering' THEN t.amount ELSE 0 END), 0) as offerings,
                        COALESCE(SUM(CASE WHEN t.transaction_type = 'donation' THEN t.amount ELSE 0 END), 0) as donations,
                        0 as pledges,
                        COALESCE(SUM(CASE WHEN t.transaction_type = 'other' AND t.amount > 0 THEN t.amount ELSE 0 END), 0) as other_income,
                        COALESCE(SUM(CASE WHEN t.transaction_type = 'expense' THEN ABS(t.amount) ELSE 0 END), 0) as operational_expenses,
                        0 as project_expenses,
                        0 as other_expenses
                    FROM transactions t
                    WHERE t.church_id = $1
            ...
            logger_1.default.error('Error in ReportRepository.getFinancialSummary:', error);
    ...
    async getEventPerformance(churchId, filters) {
        ...
        try {
            ...
            const query = `
                WITH event_stats AS (
                    SELECT
                        ...
                        COALESCE(SUM(COALESCE(ei.member_attendance, 0)), 0) as member_count,
                        COALESCE(SUM(COALESCE(ei.guest_attendance, 0)), 0) as guest_count,
            ...
            return result.rows.map(row => ({
                ...
                lastHeldDate: row.last_held_date ? (0, date_fns_1.format)(new Date(row.last_held_date), 'yyyy-MM-dd') : '',
            }));
        }
        catch (error) {
            logger_1.default.error('Error in ReportRepository.getEventPerformance:', error);
            throw new AppError_1.AppError('Failed to fetch event performance', 500);
        }
        finally {
            client.release();
        }
    ...
    // INACTIVE MEMBERS
    // ============================================================================
    async getInactiveMembers(churchId, filters = {}) {
        const { daysThreshold = 30, includeStatusInactive = true, includeNeverAttended = true, limit = 100 } = filters;
        const client = await database_1.pool.connect();
        try {
            ...
            const query = `
                WITH member_last_attendance AS (
                    ...
                    GROUP BY
                        ...
                        m.profile_image_url, m.status, m.member_type,
                        m.membership_date, m.created_at
                ),
                ...
                SELECT
                    member_id, member_name, email, phone, profile_photo,
                    status, member_type, membership_date, last_attended,
                    total_attendance_count, days_inactive, never_attended
                FROM inactive_calculation
                WHERE
                    (never_attended = true AND $3 = true AND days_inactive >= $2)
                   OR
                    (never_attended = false AND days_inactive >= $2)
                ORDER BY days_inactive DESC, member_name ASC
                    LIMIT $4
            `;
            const result = await client.query(query, [
                churchId, daysThreshold, includeNeverAttended, limit
            ]);
            return result.rows.map(row => ({
                ...
                email: row.email || '',
                phone: row.phone || '',
                lastAttended: row.last_attended ? (0, date_fns_1.format)(new Date(row.last_attended), 'yyyy-MM-dd') : null,
                daysInactive: parseInt(row.days_inactive) || 0,
                totalAttendanceCount: parseInt(row.total_attendance_count) || 0,
                membershipDate: row.membership_date ? (0, date_fns_1.format)(new Date(row.membership_date), 'yyyy-MM-dd') : null,
            ...
            logger_1.default.error('Error in ReportRepository.getInactiveMembers:', error);
            throw new AppError_1.AppError('Failed to fetch inactive members', 500);
        }
        finally {
            client.release();
        }
        ...
        try {
            ...
            return result.rows.map(row => ({
                ...
                lastAttendedDate: row.last_family_attendance
                    ? (0, date_fns_1.format)(new Date(row.last_family_attendance), 'yyyy-MM-dd')
                    : null,
                consecutiveAbsences: parseInt(row.consecutive_absences) || 0,
            }));
        }
        catch (error) {
            logger_1.default.error('Error in ReportRepository.getFamilyAttendance:', error);
            throw new AppError_1.AppError('Failed to fetch family attendance', 500);
        }
        finally {
            client.release();
        }
        ...
        try {
            ...
            const query = `
                WITH group_stats AS (
                    SELECT
                        ...
                        COUNT(DISTINCT gm.id) as active_member_count,
            ...
            return result.rows.map(row => ({
                ...
                lastMeetingDate: row.last_meeting_date
                    ? (0, date_fns_1.format)(new Date(row.last_meeting_date), 'yyyy-MM-dd')
                    : null,
            }));
        }
        catch (error) {
            logger_1.default.error('Error in ReportRepository.getGroupActivity:', error);
            throw new AppError_1.AppError('Failed to fetch group activity', 500);
        }
        finally {
            client.release();
        }
    ...
    async getServiceReport(churchId, instanceId) {
        ...
        try {
            const query = `
                    ...
                    COALESCE(
                            (SELECT SUM(ABS(t.amount))
                             ...
                             WHERE t.event_instance_id = ei.id
                               AND t.transaction_type IN ('offering', 'tithe', 'donation')),
                            0
                    ) as offerings
                FROM event_instances ei
                ...
                WHERE ei.id = $1
                  AND ei.church_id = $2
                GROUP BY ei.id, e.name, ei.instance_date, ei.total_attendance,
                         ei.member_attendance, ei.guest_attendance, ei.notes
            `;
            ...
            return {
                ...
                eventName: row.event_name,
                instanceDate: (0, date_fns_1.format)(new Date(row.instance_date), 'yyyy-MM-dd'),
                totalAttendance: parseInt(row.total_attendance) || 0,
                memberAttendance: parseInt(row.member_attendance) || 0,
                guestAttendance: parseInt(row.guest_attendance) || 0,
            ...
            logger_1.default.error('Error in ReportRepository.getServiceReport:', error);
            if (error instanceof AppError_1.AppError)
                throw error;
            throw new AppError_1.AppError('Failed to fetch service report', 500);
        }
        finally {
            client.release();
        }
    ...
    async getAllServiceReports(churchId, filters) {
        ...
        try {
            ...
            logger_1.default.error('Error in ReportRepository.getAllServiceReports:', error);
            throw new AppError_1.AppError('Failed to fetch service reports', 500);
        }
        finally {
            client.release();
        }
    ...
    // HELPER METHODS
    // ============================================================================
    getDateGroupSQL(granularity = 'monthly', dateColumn) {
        switch (granularity) {
            case 'daily': return `DATE(${dateColumn})`;
            case 'weekly': return `DATE_TRUNC('week', ${dateColumn})`;
            case 'monthly': return `DATE_TRUNC('month', ${dateColumn})`;
            case 'quarterly': return `DATE_TRUNC('quarter', ${dateColumn})`;
            case 'yearly': return `DATE_TRUNC('year', ${dateColumn})`;
            default: return `DATE_TRUNC('month', ${dateColumn})`;
        }
    }
    getPeriodLabelSQL(granularity = 'monthly', dateColumn) {
        switch (granularity) {
            case 'daily': return `TO_CHAR(${dateColumn}, 'Mon DD, YYYY')`;
            case 'weekly': return `'Week ' || TO_CHAR(${dateColumn}, 'WW, YYYY')`;
            case 'monthly': return `TO_CHAR(${dateColumn}, 'Mon YYYY')`;
            case 'quarterly': return `'Q' || EXTRACT(QUARTER FROM ${dateColumn})::text || ' ' || EXTRACT(YEAR FROM ${dateColumn})::text`;
            case 'yearly': return `TO_CHAR(${dateColumn}, 'YYYY')`;
            default: return `TO_CHAR(${dateColumn}, 'Mon YYYY')`;
        }
    }
    getGranularityInterval(granularity = 'monthly') {
        switch (granularity) {
