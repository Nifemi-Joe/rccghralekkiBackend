...
class ReportRepository {
        ...
        try {
            ...
            return result.rows.map(row => ({
                ...
                lastAttended: row.last_attended
                    ? (0, date_fns_1.format)(new Date(row.last_attended), 'yyyy-MM-dd')
                    : null,
                ...
                membershipDate: row.membership_date
                    ? (0, date_fns_1.format)(new Date(row.membership_date), 'yyyy-MM-dd')
                    : null,
    ...
    // HELPER METHODS
    // ============================================================================
    getDateGroupSQL(granularity = 'monthly', dateColumn = 'ei.instance_date') {
        switch (granularity) {
            case 'daily':
    ...
    getPeriodLabelSQL(granularity = 'monthly', dateColumn = 'ei.instance_date') {
        switch (granularity) {
            case 'daily':
