...
exports.ManualRegistrationService = void 0;
...
class ManualRegistrationService {
     ...
     */
    async searchRegistrants(filters) {
        ...
        if (includeMembers) {
            const memberQuery = `
                ...
                LIMIT $3
            `;
            const memberResult = await database_1.pool.query(memberQuery, [churchId, searchPattern, limit]);
        ...
        if (includeFirstTimers) {
            const firstTimerQuery = `
                ...
                LIMIT $3
            `;
            const firstTimerResult = await database_1.pool.query(firstTimerQuery, [churchId, searchPattern, limit]);
     ...
     */
    async getAllMembers(churchId, options) {
     ...
     */
    async getEventAttendees(churchId, eventId, options) {
     ...
     */
    async quickCheckIn(churchId, data) {
        ...
        try {
            ...
            `;
                ...
                if (!result.rows[0]) {
                    throw new AppError_1.AppError('Registration not found or already checked in', 404);
                }
            ...
            `;
                ...
                `, [data.checkedInBy, registrationId]);
                    ...
                    const qrCode = (0, helpers_1.generateQRCode)();
                    await client.query(`
            ...
            // Scenario 3: Walk-in (new guest or first-timer)
                ...
                if (data.walkIn.isFirstTimer || data.walkIn.type === 'first_timer') {
                    const firstTimerId = (0, uuid_1.v4)();
                    await client.query(`
     ...
     * Undo check-in
     */
    async undoCheckIn(churchId, registrationId) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
        ...
        `;
            ...
            if (!regResult.rows[0].checked_in) {
                throw new AppError_1.AppError('Registration is not checked in', 400);
            }
            // Update registration
            await client.query(`
            UPDATE event_registrations
     ...
     */
    async getRecentCheckIns(churchId, eventId, options) {
     ...
     */
    async getRegistrantDetails(churchId, id, type) {
        if (type === 'member') {
            const query = `
    ...
    async registerForEvent(churchId, data) {
        try {
            ...
            let guestPhone;
            let registrantName;
            let newRecordCreated;
            // Handle different registrant types
            switch (data.registrantType) {
                case 'existing_member':
                    if (!data.memberId) {
                        throw new AppError_1.AppError('Member ID is required for existing members', 400);
                    }
                ...
                case 'guest':
                    ...
                    break;
                default:
                    throw new AppError_1.AppError('Invalid registrant type', 400);
            }
            // Check for duplicate registration
            const existingRegistration = await this.checkExistingRegistration(data.eventId, memberId, guestEmail, guestPhone);
            if (existingRegistration) {
                throw new AppError_1.AppError('This person is already registered for this event', 400);
            }
            ...
            if (data.sendNotification && data.notificationChannels?.length) {
                const notificationData = {
                    ...
                    completionLink: this.generateProfileCompletionLink(churchId, newRecordCreated?.type || 'guest', newRecordCreated?.id || registration.id)
                };
            ...
            logger_1.default.info(`Manual registration completed for event ${event.name}`, {
                registrantType: data.registrantType,
                registrantName,
            ...
            logger_1.default.error('Error in registerForEvent:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async bulkRegisterForEvent(churchId, data) {
        const results = [];
     ...
     */
    async getEventRegistrationOptions(churchId, eventId) {
     ...
     */
    async resendProfileNotification(churchId, registrationId, channels) {
        ...
        const notificationData = {
            ...
            completionLink: this.generateProfileCompletionLink(churchId, registration.memberId ? 'member' : 'guest', registration.memberId || registrationId)
        };
     ...
     */
    async getEventRegistrationStats(churchId, eventId) {
        const query = `
            SELECT 
        ...
        `;
        const result = await database_1.pool.query(query, [eventId, churchId]);
        ...
        const byDateQuery = `
            ...
            LIMIT 30
        `;
        const byDateResult = await database_1.pool.query(byDateQuery, [eventId]);
        return {
    ...
    // ============================================================================
    async createMinimalMember(churchId, data) {
    ...
    async createMinimalFirstTimer(churchId, data) {
        const id = (0, uuid_1.v4)();
        const query = `
            INSERT INTO first_timers (
                id, church_id, first_name, last_name, email, phone,
    ...
    async checkExistingRegistration(eventId, memberId, email, phone) {
        let query = `
            SELECT id FROM event_registrations 
    ...
    generateProfileCompletionLink(churchId, type, id) {
        const baseUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
        const token = Buffer.from(`${churchId}:${type}:${id}`).toString('base64');
        return `${baseUrl}/complete-profile/${token}`;
    }
    async sendRegistrationNotification(channel, data) {
        const message = this.generateNotificationMessage(channel, data);
        switch (channel) {
            case 'email':
                if (!data.recipientEmail) {
                    throw new AppError_1.AppError('Email address is required for email notification', 400);
                }
            ...
            case 'sms':
                if (!data.recipientPhone) {
                    throw new AppError_1.AppError('Phone number is required for SMS notification', 400);
                }
            ...
            case 'whatsapp':
                if (!data.recipientPhone) {
                    throw new AppError_1.AppError('Phone number is required for WhatsApp notification', 400);
                }
    ...
    generateNotificationMessage(channel, data) {
...
exports.ManualRegistrationService = ManualRegistrationService;
//# sourceMappingURL=ManualRegistrationService.js.map