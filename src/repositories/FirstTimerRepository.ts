...
export class FirstTimerRepository {

    // ============================================================================
    // CREATE
    // ============================================================================

    async create(data: CreateFirstTimerDTO): Promise<FirstTimer> {
    ...
    async findAll(filters: FirstTimerFilters): Promise<PaginatedFirstTimers> {
    ...
    async update(id: string, churchId: string, data: UpdateFirstTimerDTO): Promise<FirstTimer | null> {
        const fields: string[] = [];
        const values: any[] = [];
        let paramCount = 0;
        ...
        for (const [key, dbField] of Object.entries(fieldMappings)) {
            const value = data[key as keyof UpdateFirstTimerDTO];
            if (value !== undefined) {
                paramCount++;
    ...
    async recordVisit(id: string, churchId: string, visitDate?: Date): Promise<FirstTimer | null> {
    ...
    async recordContactAttempt(id: string, churchId: string, notes?: string): Promise<FirstTimer | null> {
        let query = `
            UPDATE first_timers
    ...
    async markAsConverted(id: string, churchId: string, memberId: string): Promise<FirstTimer | null> {
        const query = `
            UPDATE first_timers
    ...
    private mapToFirstTimer(row: any): FirstTimer {
        return {
            id: row.id,
