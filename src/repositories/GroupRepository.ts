...
export class GroupRepository {
    ...
    async approveGroup(churchId: string, groupId: string, approvedBy: string): Promise<Group | null> {
        const query = `
            UPDATE groups
            SET 
        ...
        const result = await pool.query(query, [groupId, churchId, approvedBy]);
    ...
    async rejectGroup(
        ...
        const result = await pool.query(query, [groupId, churchId, rejectedBy, reason]);
    ...
    async getPendingGroups(churchId: string): Promise<Group[]> {
        const query = `
            SELECT 
                g.*,
                gt.name as group_type_name,
    ...
    async approveGroupMember(
        ...
        const result = await pool.query(query, [groupId, memberId, approvedBy]);
    ...
    async rejectGroupMember(
        ...
        const result = await pool.query(query, [groupId, memberId, rejectedBy, reason]);
    ...
    async getPendingGroupMembers(churchId: string): Promise<Array<GroupMember & { group_name: string }>> {
        const query = `
            SELECT 
                gm.*,
                g.name as group_name,
    ...
    async getPendingGroupMembersByGroup(groupId: string): Promise<GroupMember[]> {
        const query = `
            SELECT 
                gm.*,
                m.first_name,
    ...
    async getAllPendingApprovals(churchId: string): Promise<PendingApprovalsResponse> {
