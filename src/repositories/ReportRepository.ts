...
export class ReportRepository {
    ...
    async getInactiveMembers(
        ...
        try {
            ...
            return result.rows.map(row => ({
                ...
                email: row.email || '',
                phone: row.phone || '',
                lastAttended: row.last_attended ? format(new Date(row.last_attended), 'yyyy-MM-dd') : null,
                daysInactive: parseInt(row.days_inactive) || 0,
                totalAttendanceCount: parseInt(row.total_attendance_count) || 0,
                membershipDate: row.membership_date ? format(new Date(row.membership_date), 'yyyy-MM-dd') : null,
    ...
    private getDateGroupSQL(granularity: string = 'monthly', dateColumn: string): string {
        switch (granularity) {
    ...
    private getPeriodLabelSQL(granularity: string = 'monthly', dateColumn: string): string {
        switch (granularity) {
