...
export class NotificationService {
    ...
    private initializeTwilioClient(): void {
        try {
            if (process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN) {
                ...
                logger.warn('Twilio configuration incomplete - SMS will be logged only');
    ...
    async notifyChurchAdmins(data: NotificationData): Promise<void> {
        try {
            ...
            const notificationInserts = adminsResult.rows.map(admin =>
                this.createInAppNotification(admin.id, data)
            );
     ...
     */
    async notifyExpenseSubmitter(
        userId: string,
        churchId: string,
        data: Omit<NotificationData, 'churchId'>
    ): Promise<void> {
        try {
            ...
            logger.error('Error notifying expense submitter:', error);
        }
    }
    ...
    async sendEmail(options: EmailOptions): Promise<void> {
        ...
        if (!this.isEmailConfigured || !this.emailTransporter) {
            ...
            logger.debug(`[EMAIL MOCK] Content: ${text || htmlContent?.substring(0, 200)}...`);
            return;
        }
    ...
    async sendSMS(options: SMSOptions): Promise<void> {
        ...
        if (!this.isSMSConfigured || !this.twilioClient) {
            logger.info(`[SMS MOCK] To: ${cleanPhone}, Message: ${message.substring(0, 100)}...`);
            return;
        }
        ...
        try {
            ...
            logger.error(`Failed to send SMS to ${cleanPhone}:`, error);
    ...
    async sendWhatsApp(options: WhatsAppOptions): Promise<void> {
        ...
        if (!this.isSMSConfigured || !this.twilioClient || !process.env.TWILIO_WHATSAPP_NUMBER) {
            logger.info(`[WHATSAPP MOCK] To: ${cleanPhone}, Message: ${message.substring(0, 100)}...`);
            return;
        }
        ...
        try {
            ...
            logger.error(`Failed to send WhatsApp to ${cleanPhone}:`, error);
     ...
     */
    async createInAppNotification(userId: string, data: NotificationData): Promise<void> {
        try {
            await pool.query(
            ...
            logger.debug(`In-app notification created for user ${userId}`);
        } catch (error) {
    ...
    private getSubjectForType(type: NotificationType, defaultTitle: string): string {
    ...
    private formatSMSMessage(type: NotificationType, message: string, actionUrl?: string): string {
    ...
    private renderEmailTemplate(templateName: string, data: Record<string, any>): string {
    ...
    private getBaseTemplate(content: string, data: Record<string, any>): string {
    ...
    private profileUpdateLinkTemplate(data: Record<string, any>): string {
    ...
    private memberWelcomeTemplate(data: Record<string, any>): string {
        const content = `
            <div class="header">
                <div class="logo">👋</div>
                <h1>Welcome to ${data.churchName || 'Our Church'}!</h1>
    ...
    private birthdayTemplate(data: Record<string, any>): string {
        const content = `
    ...
    private anniversaryTemplate(data: Record<string, any>): string {
        const content = `
    ...
    private eventReminderTemplate(data: Record<string, any>): string {
        const content = `
            <div class="header">
                <div class="logo">📅</div>
    ...
    private eventRegistrationTemplate(data: Record<string, any>): string {
        const content = `
            <div class="header">
                <div class="logo">✅</div>
    ...
    private expenseTemplate(data: Record<string, any>, status: 'pending' | 'approved' | 'rejected'): string {
        const statusConfig = {
            pending: {
                icon: '📋',
    ...
    private passwordResetTemplate(data: Record<string, any>): string {
        const content = `
            <div class="header">
                <div class="logo">🔐</div>
    ...
    private accountVerificationTemplate(data: Record<string, any>): string {
        const content = `
            <div class="header">
                <div class="logo">📧</div>
    ...
    private generalTemplate(data: Record<string, any>): string {
        const content = `
            <div class="header">
     ...
     */
    async verifyEmailConfiguration(): Promise<{ success: boolean; message: string }> {
        if (!this.isEmailConfigured || !this.emailTransporter) {
